...
def init(WF):
    """Function that initializes dictionnaries used as part of the wake tree model (windfarm model) and the core DWM model
        ...
        Inputs
        ----------
        WF (instance of class): Instance of class WindFarm holding wind farm layout and coordinate
       ...
       out: dict(nWT), holding the main sDWM model outputs i.e mean power from BEM, mean power estimated from power curve,
        mean rotor averaged wind speed, mean rotor average turbulence intensity, mean thrust coefficient from BEM and from power curve
       DWM: dict(nWT)   list containing full outputs of the sDWM (including flow field in ffor and mfor) See description of DWM_outputs for more details
       ID_waked: dict(nWT) holding list of upstream turbine index for each turbine in the wind farm
       ID_wake_adj: dict(nWT) holding list of downstream turbine index for each turbine in the wind farm
       Farm_p_out (float) : wind farm power produced
       WT_p_out (np.array float): power produced by each turbine in the wind farm
       Vel_out (np.array float): mean rotor averaged aggregated velocity for each turbine in the wind farm
       Pos_out (np.array float): outputs coordinates X,Y of wind turbines
    """
